...
class UserTest < ActiveSupport::TestCase
  ...
  context "Creating a context for users" do
    ...
    should "have working class method for authenication" do 
      assert User.authenticate("ed@example.com", "creamery")
      deny User.authenticate("ed@example.com", "password")
    end
    ...
    should "require a password for new users" do
      bad_user = FactoryGirl.build(:user, email: "sed@example.com", password: nil)
      deny bad_user.valid?
    end
    
    should "require passwords to be confirmed and matching" do
      bad_user_1 = FactoryGirl.build(:user, email: "sed@example.com", password: "secret", password_confirmation: nil)
      deny bad_user_1.valid?
      bad_user_2 = FactoryGirl.build(:user, email: "sed@example.com", password: "secret", password_confirmation: "sauce")
      deny bad_user_2.valid?
    end
    
    should "require passwords to be at least four characters" do
      bad_user = FactoryGirl.build(:user, email: "sed@example.com", password: "no")
      deny bad_user.valid?
    end
